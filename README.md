...
한국과 일본 각각 **별도 cron 설치** — 동시 실행 충돌 회피를 위해 기본 시각이 분리되어 있음 (한국 04:00 / 일본 05:30).
...
```bash
# 기본 — 매일 05:30 (한국 04:00 과 시간 분리)
./crawler/install_jp_cron.sh

# 옵션
SCHEDULE="30 6 * * *" ./crawler/install_jp_cron.sh   # 시간 변경
PREFECTURES=13,27 ./crawler/install_jp_cron.sh        # 특정 도도부현만 (東京·大阪)
MAX_PAGES=20 ./crawler/install_jp_cron.sh             # 도도부현당 페이지 상한

# 상태 / 제거 / 수동 실행
./crawler/install_jp_cron.sh show
./crawler/install_jp_cron.sh uninstall
./crawler/run_jp_daily.sh   # 즉시 1회
```

#### 공통

`run_daily.sh` / `run_jp_daily.sh` 가 매 실행 시 `.env`를 자동 source — 새 키 추가만 하면 cron 재설치 불필요. 로그는 `crawler/data/logs/daily_*.log` / `jp_daily_*.log` (30일 자동 정리), 동시 실행은 mkdir 락으로 방지.

> ⚠️ macOS는 노트북이 sleep 시 cron 미동작. 항상-켜진 환경(iMac/서버) 권장. Privacy & Security → Full Disk Access에 `cron` 추가가 필요할 수 있음.

#### 갱신 동작 확인

iMac 등 다른 머신에서 cron이 돌고 있는지는 Supabase에서 SQL로 확인할 수 있습니다 (한국·일본 모두 `crawl_runs` 공통 테이블 사용). 검증 쿼리는 [docs/ops_monitoring.md](docs/ops_monitoring.md) 참조.
