...
```bash
# .env 에서 MinIO 자격증명 읽기 (셸에 없으면 빈 값 → 인증 실패)
MINIO_ROOT_USER=$(grep '^MINIO_ROOT_USER=' .env | cut -d= -f2-)
MINIO_ROOT_PASSWORD=$(grep '^MINIO_ROOT_PASSWORD=' .env | cut -d= -f2-)

# mc 컨테이너로 1회 실행.
# 주의: minio/mc 는 entrypoint 가 'mc' 라서 'sh -c' 를 직접 못 받음.
#       --entrypoint /bin/sh 로 덮어쓰고 -c "..." 전달.
...
## 8. 외부 노출 — DSM 역방향 프록시 + synology.me (경로 기반)

synology.me 는 서브도메인을 못 만들므로 **단일 호스트 + 경로 기반**. DSM 이 TLS 종단
(Let's Encrypt), Caddy 가 경로 분기(`/rest/v1`→PostgREST, `/storage`→MinIO).

**8-1. 인증서** — DSM → 제어판 → 보안 → 인증서 → 추가 → Let's Encrypt
- 도메인 `jeremylab.synology.me` + 이메일. 발급 후 자동 갱신.

**8-2. 역방향 프록시** — DSM → 제어판 → 로그인 포털 → 고급 → 역방향 프록시 → 생성
- 설명 `auction`
- 소스: **HTTPS / `jeremylab.synology.me` / 443**
- 대상: **HTTP / `localhost` / 8080**  (Caddy)
- 저장 후 제어판 → 보안 → 인증서 → 설정 에서 이 서비스에 8-1 인증서 지정.

**8-3. 라우터 포트포워딩** — 외부 `443` → `192.168.50.201:443`.

> 외부 진입점은 **`https://jeremylab.synology.me`** 하나. supabase-js 가 `/rest/v1/*`,
> 이미지는 `/storage/{bucket}/{key}`.

## 9. 검증

```bash
ANON="<7번 ANON_KEY 통째로>"

# 내부 — NAS IP 직접 (Caddy)
curl -H "Authorization: Bearer $ANON" \
  "http://192.168.50.201:8080/rest/v1/courts?select=code,name&limit=3"

# 외부 — DSM 역방향 프록시 경유 (8장 완료 후)
curl -H "Authorization: Bearer $ANON" \
  "https://jeremylab.synology.me/rest/v1/courts?select=code,name&limit=3"
```

빈 배열 `[]` 이 정상 (아직 데이터 없음). `curl: (3) Error` 면 URL 의 `<...>` 를
실제 값으로 안 바꾼 것. 401 → ANON↔JWT_SECRET 불일치, 404 → 마이그레이션 미적용.

## 10. 크롤러 / 웹 환경변수

### iMac 크롤러 — `/Users/.../auction/.env` (LAN 내부 IP 직접)
```dotenv
SUPABASE_URL=http://192.168.50.201:8080
SUPABASE_KEY=<ANON_KEY>
SUPABASE_SERVICE_KEY=<SERVICE_KEY>     # 적재용
MINIO_ENDPOINT=http://192.168.50.201:9000
...
```
SUPABASE_URL=https://jeremylab.synology.me
SUPABASE_KEY=<ANON_KEY>
STORAGE_PUBLIC_URL=https://jeremylab.synology.me/storage
